...
// mapToStruct converts a map[string]interface{} to a typed value T.
//
// mapToStruct uses JSON marshal/unmarshal for maximum compatibility and flexibility
// with custom types, embedded fields, and field tags. This is the foundation for all
// generic conversion methods: CreateAs, UpdateAs, CollectAs, StreamAs, FindByKeyAs, FirstAs.
//
...
// This is used internally by [StreamAs] for optimal streaming performance.
//
// Compared to [mapToStruct], this method skips the JSON marshaling step and avoids
...
func rawMessageToStruct[T any](raw json.RawMessage) (T, error) {
	...
	// Unmarshal JSON bytes directly into the target type T
	// This skips the map[string]interface{} intermediate step
...
// CreateAs creates a new entity and decodes the response to type T.
//
// CreateAs is the generic version of [Client.Create]. It sends a POST request
...
//	order, err := CreateAs[Order](client, ctx, "Orders", newOrder)
//	// order.ID is now set by the server
func CreateAs[T any](c *Client, ctx context.Context, entitySet string, data interface{}) (T, error) {
...
// FindByKeyAs retrieves a single entity by its key and decodes it to type T.
//
// FindByKeyAs is the generic version of [QueryBuilder.FindByKey]. It constructs
...
//	}
//
//	customer, err := FindByKeyAs[Customer](qb, ctx, 42)
//	// or with composite key:
//	customer, err := FindByKeyAs[Customer](qb, ctx, map[string]interface{}{"CompanyID": 1, "CustomerID": 42})
...
// CollectAs materializes all results from the query into a slice of type T.
//
...
func CollectAs[T any](qb *QueryBuilder, ctx context.Context) ([]T, error) {
	...
	for result := range qb.Stream(ctx) {
		...
		item, err := mapToStruct[T](result.Value)
		if err != nil {
			return nil, err
		}
...
// FirstAs retrieves the first result from the query and decodes it to type T.
//
// FirstAs is the generic version of [QueryBuilder.First]. It executes the query
...
//
//	result, _ := FirstAs[T](qb, ctx)
//	// vs
//	item, _ := qb.Top(1).First(ctx)
//	item, _ := mapToStruct[T](item)
//
...
//	}
//
//	customer, err := FirstAs[Customer](qb.Filter("Name eq 'Alice'"), ctx)
...
// StreamAs is the generic streaming method for type T.
//
// StreamAs returns a channel of [Result] items typed to T, enabling incremental
...
// avoiding the intermediate map[string]interface{} allocation required by [CollectAs].
...
//	results := StreamAs[Product](qb.Filter("Price gt 50"), ctx, 512)
...
func StreamAs[T any](qb *QueryBuilder, ctx context.Context, bufferSize ...int) <-chan Result[T] {
	...
	go func() {
		...
		// Use the optimized raw stream for direct type conversion
		for result := range qb.streamRaw(ctx, buffer) {
			...
			// Unmarshal directly from raw JSON without intermediate map
